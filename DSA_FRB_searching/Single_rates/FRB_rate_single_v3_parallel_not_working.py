...
@author: gechen
FRB_rate_single_v3_parallel.py: computes the detection rate of a single FRB. 

V1: rate vs. time resolution 
V2: adds grid plot for rate vs. (time resolution, channel width)
V3: adds additional dispersion smearing due to the DM trail step size. Integration becomes slow. 
V3_parallel: speed up the integration by using parallel processing. ('Number of processors: ', 4) 
"""
import numpy as np 
...
def Power_response(theta):
    '''
    Single dish power response in 1-D.
    '''
    return stats.norm.pdf(theta, 0, 3.3/2) 
...
def Compute_w_eff(w_int, DM, channel_number, time_resolution, DM_min, DM_max, include_w_delta_DM = True):
    '''
...
def Compute_F0(w_int, DM, time_resolution, channel_number, DM_min, DM_max, S2N_min=8):
    ...
    # for debug
    with open('Integration_v3.txt', 'a') as f: 
...
def Compute_detection_rate(time_resolution, channel_number, DM_min, DM_max,f=Rate_integrand): 
    ...
    results = [pool.apply(integrate.dblquad, args=(f, 0, np.inf, lambda x: DM_min, lambda x: DM_max, args=[time_resolution, channel_number, DM_min, DM_max], epsabs=1e-2, epsrel=1e-2)]     
    pool.close()  
    return results
    #return integrate.dblquad(f, 0, np.inf, lambda x: DM_min, lambda x: DM_max, args=[time_resolution, channel_number, DM_min, DM_max], epsabs=1e-2, epsrel=1e-2) 
